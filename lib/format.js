...
function formatVariant (key, typeDef, variant, trimSelf) {
	...
	if (isFunction) {
		...
		if (typeDef.returnTypes) {
			let retTypes = "";
			typeDef.returnTypes.forEach((type,index) => {
				if(type){
					retTypes += type.actualType + (index == typeDef.returnTypes.length - 1 ? '' : ', ');

				}else{
					retTypes += 'unknown' + (index == typeDef.returnTypes.length - 1 ? '' : ', ');
				}
			});
...
export default function formatResults (results, trimSelf) {
	...
	results.forEach(({ key, typeDef }) => {
		if (typeDef.type === 'function') {
			if (trimSelf && (typeDef.canSelfCall || typeDef.onlySelfCall)) {
				if (typeDef.variants) {
			...
			} else if ( trimSelf && typeDef.onlySelfCall ) {
				if (typeDef.variants) {
					typeDef.variants.forEach(variant =>
						suggestions.push(formatVariant(key, typeDef, variant, trimSelf))
					)
