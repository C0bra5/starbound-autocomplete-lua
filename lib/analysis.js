		...
		return await this._evaluate(() => {
			...
			if (this.queryBase) {
				if (config.preventDefinitionOverrides) {
					if (this.queryBase.globalScope.fields[this.queryBase.name] && this.queryBase.scope.fields[this.queryBase.name] && this.queryBase.scope.fields[this.queryBase.name].fields) {
