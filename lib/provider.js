...
export default class LuaProvider {
	...
	constructor () {
		...
		this.disposeables.push(window.atom.config.observe('starbound-autocomplete-lua.inclusionPriority', (value) => {
	...
	getSuggestions = async function (request) {
		...
		try {
			...
			analysis.write(buffer.getTextInRange([
				buffer.positionForCharacterIndex(continuePos),
